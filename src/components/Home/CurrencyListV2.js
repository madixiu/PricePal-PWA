  ...
  return CurrencyData.map((item) => (
    ...
    >
      ...
      <Box sx={{display:'flex',flex:1,justifyContent: 'center',alignItems:'center'}}>
        <Typography sx={typographyListStyle}>{document.body.dir === 'ltr' ? formatPrice(item.prices.buy.price):FarsiDigitPrice(item.prices.buy.price)}</Typography>
      </Box>
      <Box sx={{display:'flex',flex:1,justifyContent: 'center',alignItems:'center'}}>
        <Typography sx={typographyListStyle}>{document.body.dir === 'ltr' ? formatPrice(item.prices.sell.price): FarsiDigitPrice(item.prices.sell.price)}</Typography>
      </Box>
      <Box sx={{display:'flex',flex:1,justifyContent: 'center',alignItems:'baseline',flexWrap:'wrap'}}>
        ...
        <Typography sx={[typographyListStyle,{color:ChangeColor(item.change_direction)}]}>{item.change_percentage === 0 ? "-" : document.body.dir === 'ltr' ? formatPrice(item.change_percentage): FarsiDigitPrice(item.change_percentage)}</Typography>
      </Box>
      <Box sx={{display:'flex',flex:1,justifyContent: 'center',alignItems:'center'}}>
        <Typography sx={typographyListStyle}>{document.body.dir === 'ltr' ? formatTime(item.lastUpdate) : formatTimeFarsiDigit(item.lastUpdate)}</Typography>
      </Box>
    </Box>
  ));
}
